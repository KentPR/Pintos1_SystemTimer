...
{
  struct thread *thr;     //поток
  int64_t sleep_ticks;    //тики сна
  struct thr_queue *next; //просто следующий
};

static struct thr_queue *head = NULL; //указатель на голову списка
static struct thr_queue *tail = NULL; // указатель на хвост списка
...
void create_node_insert(int64_t s_ticks)
{
  ...
  if (head == NULL)
  {
    head = (struct thr_queue *)malloc(sizeof(struct thr_queue));
    head->next = NULL;
    tail = head;
  ...
  {
    if (head->sleep_ticks > s_ticks) // обрабатываем голову
    {
      head = (struct thr_queue *)malloc(sizeof(struct thr_queue));
      head->thr = thread_current();
      head->sleep_ticks = s_ticks;
    ...
    if (tail->sleep_ticks < s_ticks) // обрабатываем хвост
    {
      tail->next = (struct thr_queue *)malloc(sizeof(struct thr_queue));
      tail = tail->next;
      tail->sleep_ticks = s_ticks;
      tail->thr = trhread_current();
      tail->next = NULL;
      return;
    }
    //вставка внутрь тела списка
    while (cur->next->sleep_ticks <= s_ticks) //ищем нужное место
    {
      cur = cur->next; //переход по списку
    }
    tmp = cur->next;
    cur->next = (struct thr_queue *)malloc(sizeof(struct thr_queue));
